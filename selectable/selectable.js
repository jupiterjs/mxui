...
/**
 * Provides keyboard and mouse selection (and multi selection).
 * 
 * $('#element').mxui_selectable();
 */
$.Controller.extend('Mxui.Selectable',{
    defaults : {
		...
		multiActivate: true,
		cache : false,
		//uses models on activate ... sends only 1 activate message with multiple models
		models : false
    }
},
{
	...
	_activate : function(el, ev){
		...
		if(!this.options.multiActivate || (!ev.shiftKey && !ev.ctrlKey)){
			...
			el.trigger("activate",[activated.addClass(this.options.activatedClassName).models()]);
